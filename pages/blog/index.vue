<template>
  <div>
      ...
      <article v-for="blog in blogs" :key="blog.id" class="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
        ...
        <div class="p-6">
          ...
          <h2 class="text-2xl font-bold mb-4">
            <NuxtLink :to="`/blog/${blog.slug}`" class="hover:text-blue-400 transition-colors">
              {{ blog.title }}
            </NuxtLink>
          </h2>
    ...
    <div v-if="blogs.length > 0 && blogs.length % 10 === 0" class="text-center mt-12">
      <button
        @click="loadMoreBlogs"
...
console.log('Blog index component mounted.' + blogs.value.length + ' blogs currently loaded.');
